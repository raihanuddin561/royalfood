...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE lower(typname) = lower('UserRole')) THEN
    EXECUTE 'CREATE TYPE "UserRole" AS ENUM (''ADMIN'',''MANAGER'',''EMPLOYEE'',''CUSTOMER'')';
  END IF;
END$$;
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE lower(typname) = lower('PayrollStatus')) THEN
    EXECUTE 'CREATE TYPE "PayrollStatus" AS ENUM (''PENDING'',''APPROVED'',''PAID'')';
  END IF;
END$$;
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE lower(typname) = lower('InventoryLogType')) THEN
    EXECUTE 'CREATE TYPE "InventoryLogType" AS ENUM (''STOCK_IN'',''STOCK_OUT'',''ADJUSTMENT'',''WASTE'',''TRANSFER'')';
  END IF;
END$$;
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE lower(typname) = lower('PurchaseStatus')) THEN
    EXECUTE 'CREATE TYPE "PurchaseStatus" AS ENUM (''PENDING'',''RECEIVED'',''CANCELLED'')';
  END IF;
END$$;
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE lower(typname) = lower('OrderType')) THEN
    EXECUTE 'CREATE TYPE "OrderType" AS ENUM (''DINE_IN'',''TAKEAWAY'',''DELIVERY'')';
  END IF;
END$$;
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE lower(typname) = lower('OrderStatus')) THEN
    EXECUTE 'CREATE TYPE "OrderStatus" AS ENUM (''PENDING'',''CONFIRMED'',''PREPARING'',''READY'',''SERVED'',''COMPLETED'',''CANCELLED'')';
  END IF;
END$$;
DO $$
BEGIN
  ...
  DO $$
  BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE lower(typname) = lower('ExpenseType')) THEN
      EXECUTE 'CREATE TYPE "ExpenseType" AS ENUM (''OPERATIONAL'',''STOCK'',''PAYROLL'',''UTILITIES'',''RENT'',''MARKETING'',''MAINTENANCE'',''INSURANCE'',''TAXES'',''OTHER'')';
    END IF;
  END$$;
  DO $$
  BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE lower(typname) = lower('ExpenseStatus')) THEN
      EXECUTE 'CREATE TYPE "ExpenseStatus" AS ENUM (''PENDING'',''APPROVED'',''REJECTED'',''PAID'')';
    END IF;
  END$$;
  DO $$
  BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE lower(typname) = lower('RecurringPeriod')) THEN
      EXECUTE 'CREATE TYPE "RecurringPeriod" AS ENUM (''DAILY'',''WEEKLY'',''MONTHLY'',''QUARTERLY'',''YEARLY'')';
    END IF;
  END$$;
  DO $$
  BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE lower(typname) = lower('ReportType')) THEN
      EXECUTE 'CREATE TYPE "ReportType" AS ENUM (''DAILY'',''WEEKLY'',''MONTHLY'',''QUARTERLY'',''YEARLY'',''CUSTOM'')';
    END IF;
  END$$;
